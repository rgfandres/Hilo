-- A26 · Las plantillas de sector pueden crear un proveedor que no se asigna (p. ej. el cortador).
...
begin
  ...
  for x in select value as j from jsonb_array_elements(coalesce(p->'proveedores', '[]'::jsonb)) loop
    if jsonb_typeof(x.j) = 'object' then
      insert into proveedor (tienda_id, nombre, tipo, unidad_pedido, asignable)
      values (v_tienda, x.j->>'nombre', coalesce(x.j->>'tipo', 'ENCARGOS'), (x.j->>'unidad_pedido')::numeric, coalesce((x.j->>'asignable')::boolean, true));
    else
