-- 0051 · Solo la temporada activa y fecha de las comprobaciones
-- 1) Con ajustes.solo_periodo_activo, el portal del proveedor solo enseña lo del periodo activo
--    (la app hace lo mismo en sus listas).
-- 2) cambiar_fecha_check: administración u operativo pueden corregir la fecha de una comprobación
--    marcada (p. ej. «Adorno comprado»), igual que la de un paso.
...
begin
  ...
  select coalesce((ajustes->>'solo_periodo_activo')::boolean, false) into v_solo from tienda where id = p_tienda;
  select id into v_per from periodo where tienda_id = p_tienda and activo limit 1;
  return query
  with base as (
...
create or replace function cambiar_fecha_check(p_encargo uuid, p_clave text, p_fecha timestamptz)
...
begin
  ...
  update check_encargo set fecha = p_fecha where encargo_id = p_encargo and clave = p_clave and marcado;
  if not found then raise exception 'Esa comprobación no está marcada'; end if;
end $$;
revoke all on function cambiar_fecha_check(uuid, text, timestamptz) from public, anon;
grant execute on function cambiar_fecha_check(uuid, text, timestamptz) to authenticated;
